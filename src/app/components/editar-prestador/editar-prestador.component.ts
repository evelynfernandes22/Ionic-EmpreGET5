...
export class EditarPrestadorComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.apiService.getFotoByIdListaPrestadores(this.apiService.readPrestadorId())
    .pipe(
      catchError((error) => {
        if (error.status === 404) {
          console.log('Foto não encontrada no servidor.');
    ...
    .subscribe((data) => {
      if(data !== null && data.nomeArquivo){
        ...
        const imgPadrao = this.defaultPhotoUrl;
        this.imgUrl = imgPadrao;
      }
    },
    (error) => {      
